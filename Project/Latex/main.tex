...
\begin{titlepage}
  ...
  \begin{tabular}{@{}lcl@{}}
    ...
    \midrule
    Enderborg, Eric & 920816-2637 & \texttt{ericend@kth.se} \\
    \bottomrule
  \end{tabular}
...
% ══════════════════════════════════════════════════════════════════════════════
\section{Mathematical Model and Numerical Methods}
\subsection{Part 1 -- Heat Exchanger}
We are tasked with determining a numerical value of the outer
tube diameter $d$ that fulfils the specification of the other
...
\begin{equation}
  ...
  \label{eq:Uf}
\end{equation}
Here, $d$ is the outer tube diameter which is as of yet unknown and, $D_s = 1.219\ \mathrm{m}$
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}ccc@{}}
    ...
    \bottomrule
  \end{tabular}
  \caption{Material and Thermodynamic Parameter values.}
  \label{tab:params1}
\end{table}
...
This is implemented and solves for the unknown $d$ using the python code in \cref{lst:NM-eq}.
% ══════════════════════════════════════════════════════════════════════════════
...
\centering
\caption{Convergence results for the nonlinear system of equations solved using Newton's method as defined in \cref{alg:newton_system}. The table displays the iterates $(d_k, D_{s,k})$, the $2$-norm of the residual vector $\lVert \mathbf{F}(\mathbf{x}_k) \rVert_2$, the error relative to the final solution $\lVert \mathbf{x}_k - \mathbf{x}^* \rVert_2$, and the observed order of convergence $p_k$. See \cref{lst:NM-sys} for the corresponding Python implementation.}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\linewidth}
        ...
        \caption{Convergence evolution of the Newton system, showing the error at the next iteration, $\lVert \mathbf{x}_{k+1} - \mathbf{x}^* \rVert_2$, as a function of the current error, $\lVert \mathbf{x}_k - \mathbf{x}^* \rVert_2$ illustrating linear convergence. See \cref{lst:NM-sys} for the implementation.}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.65\linewidth}
        ...
        \caption{Temperature distribution of approximate Newton solution compared to the analytical case \cref{eq:infty-analytical} showing convergence.}
        \label{fig:heatsink-inftyT}
    \end{minipage}
    \begin{minipage}{0.65\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Plots/8_3a_error.png}
        \caption{Pointwise error of approximate Newton solution compared to the analytical case \cref{eq:infty-analytical} showing early discrepancy due to boundary conditions.}
...
\begin{figure}[H]
    ...
    \caption{Discrete 2-norm error as a function of N showing $N^{-2}$ proportionality.}
...
Let $\theta(L)$ and $\theta(b)$ be the temperature at $x = L$ and $x = 0$ respectively, we assume that:
\begin{equation}
	\theta(L) = \theta_b e^{-\sqrt{\alpha_1} L} \implies \frac{\theta(L)}{\theta_b} = e^{-\sqrt{\alpha_1} L}
\end{equation}

We wish this ratio to be $\leq 1 \%$. Solving for $L$ we get: 
\begin{equation}
	L > \frac{-4.605}{-\sqrt{\alpha_1}} \approx \frac{4.6}{\sqrt{\alpha_1}}
	\label{eq:infinite-criterion}
\end{equation}

Thus, for a heat sink to satisfy the infinite-length assumption, its length must exceed approximately $4.6/\sqrt{\alpha_1}$.
...
Using \cref{lst:heatsinks} we calculate the full temperature distribution for several lengths for each of the different materials using Newton's Method as described in \cref{solving-the-system}, the results are presented in \cref{fig:T_SS}, \cref{fig:T_Al}, \cref{fig:T_Cu} and confirm our earlier first estimates that the approximation is valid for all the materials except copper.
...
\begin{figure}[h!]
	...
	\caption{Temperature distribution \(T(x)\) for AISI316, Aluminium and Copper, illustrating how the temperature distribution for different lengths of the heat sink diverges or converges with the infinite-length approximation.}
...
The numerical simulations performed in this project demonstrate that Newton’s method is a highly effective tool for solving the non-linear problems associated with thermal design, provided that the initial guesses are chosen within the appropriate local convergence neighbourhoods.


In the heat exchanger analysis, we observed that the scalar formulation of the design problem yields two mathematically valid roots for the outer tube diameter $d$. 


By choosing the larger of the two roots, we satisfy the requirements while optimizing the design for lower material usage and reduced manufacturing complexity illustrating that careful consideration of the physics of the problem is crucial to be able to choose the final solution. 


While the scalar version of the method converges quadratically in most cases, Newton's method for the $(d, D_s)$ pair showed more complex convergence behaviour, likely linked to the poor conditioning of the Jacobian near the solution, highlighting the need for well behaved systems for Newton's Method.


For the heat sink analysis, the finite difference approximation was validated through a comparison with an analytical solution for an infinitely long heat sink.
The error analysis confirmed that the implementation is globally second-order accurate ($\mathcal{O}(h^2)$), as demonstrated by the convergence order $p \approx 2$ observed.


Furthermore, the simulation of three distinct materials illustrated that thermal conductivity ($K$) is most likely the dominant factor in heat sink efficiency; materials with low conductivity, such as stainless steel, trap heat near the base, whereas high-conductivity materials like copper disperse heat very effectively making it very effective as a heat sink material.


The evaluation of the “infinite length” assumption confirms that this design approximation is valid for stainless steel and aluminium fins of $\approx 0.30$ m length. However, for copper or other high conductivity materials, the resulting model shows insufficient temperature decay at the tip, indicating that the  infinite-length formula would significantly underestimate the actual temperature distribution and is therefore not useful in such cases.


These findings highlight the importance of not only choosing the right numerical method but also validating when simplified theoretical models are appropriate for practical engineering applications.

\begin{thebibliography}{9}
	
	\bibitem{ali-kara} 
	Y. Ali Kara, Ö. Güraras, \textit{A computer program for designing of shell-and-tube heat exchangers}, Appl. Therm. Eng. 24, pp. 1797–1805 (2004).
	
	\bibitem{incropera} 
	Frank P. Incropera, David P. Dewitt, Theodore L. Bergman, Andrienne S. Levine, \textit{Principles of Heat and Mass Transfer}, Seventh Edition, 2013 John Wiley \& Sons, pp. 112–228.
	
	\bibitem{sauer} 
	Timothy Sauer, \textit{Numerical Analysis}, Third Edition, 2018 Pearson Education.
...
\lstinputlisting[caption={Heat Sink -- Newton's Method for non-linear system},
                 label={lst:heatsinks}]{Code/2_2.py}
