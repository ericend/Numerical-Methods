...
def run_83() -> None:
    ...
    b: float = 0.0  # alpha_2 = 0, linear problem
    ...
    # Order of accuracy: eN ≈ C·h^p = C·(L/N)^p -> p ≈ log(e_k/e_{k+1}) / log(N_{k+1}/N_k)
    orders = [None] + [
...
def run_84() -> None:
    ...
    T_L: float = T_inf
    L_84a: float = 0.30  # fin length for 8.4.a
    N: int = 400

    # ------------- Table 4 -------------
    materials = ["SS AISI 316", "Aluminium (Al)", "Koppar (Cu)"]
    ...
    ax4.set_title(r"Temperature distribution for different fin materials ($N=400$)")
    ax4.legend(frameon=False)
    fig4.tight_layout()
    fig4.savefig(plot_dir / "8_4a_materials.png", bbox_inches="tight")

    # ------------- 8.4.b Results -------------
    tol_frac = 0.01  # e.g. 1% relative profile error
    ...
    for mat in materials:
        ...
        rel_errs = {}
        for L in Ls_sorted[:-1]:  # exclude L_ref itself
        ...
        print(f"  -> Estimated L_min ≈ {L_min} m")
